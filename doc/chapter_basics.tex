...
\end{gather*}
oraz $C \in \mathbb{N} \wedge n > 1$ jest liczbą kanałów z przestrzeni barw. \\
...
\end{gather*}
gdzie $M$, $N$ oznaczają odpowiednio szerokość i wysokość obrazu, natomiast $C$ oznacza liczbę kanałów w obrazie.\\
Wartość funkcji $f(x, y, c)$ nazywamy \textbf{pikselem}. Trzeci argument może być pominięty dla obrazów jednokanałowych, ponieważ w takiej sytuacji zawsze przyjmuje wartość 1.\\
...
\begin{gather*}
  f^{(out)}(x, y, c) = \displaystyle\sum_{i=1}^{n} f^{(i)}(x, y), \quad (x, y, c) \in P^{(in)}_{N,M},\ c \in \{1, 2, ..., C\}.
\end{gather*}
